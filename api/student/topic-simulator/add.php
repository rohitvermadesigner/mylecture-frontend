...
if (count($error_msgs) == 0) {
    ...
    if ($token_details['status'] == 'success') {
        ...
        if (
            ...
            $query = "SELECT * FROM topic_simulator_test_config WHERE name = '$test_name' AND created_by = '$student_id' AND status = 1";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                $where_condition .= " AND topic_id = $topic_id";
                if (!empty($keyword)) {
                ...
                if ($total_db_rows ==  $total_questions) {
                    ...
                    $query = "INSERT INTO topic_simulator_test_config 
                        ...
                        VALUES 
                        ('$test_name', '$duration', '$is_question_random_order', '$is_mandatory_all_question', '$subject_id', '$topic_id', '$total_questions', '$marks_for_correct_question', '$student_id', '$current_date')";
                    $result = mysqli_query($db, $query);
                    $test_id = "0";
                    ...
                    while ($question_data = mysqli_fetch_assoc($questionResult)) {
                        ...
                        $insertQuery .=  "INSERT INTO topic_simulator_test_question 
                    ...
                    http_response_code(400);
                    echo json_encode(array(
                        "message" => "We found $total_db_rows questions in our database please select different subject, chapter, topic or change number of questions",
                    ));
                    exit;
                }
            }
