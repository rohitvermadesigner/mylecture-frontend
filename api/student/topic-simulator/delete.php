...
if (count($error_msgs) == 0) {
    ...
    if ($token_details['status'] == 'success') {
        ...
        if (!empty($post_data->test_id)) {
            ...
            $query = "SELECT * FROM topic_simulator_test_config WHERE id = '$test_id' AND created_by = '$student_id'";
            $result = mysqli_query($db, $query);
            if (mysqli_num_rows($result) > 0) {
                $current_date = date('Y-m-d H:i:s');
                $query = "UPDATE topic_simulator_test_config SET status = '0', deleted_at = '$current_date' WHERE id = $test_id";
                $result = mysqli_query($db, $query);
