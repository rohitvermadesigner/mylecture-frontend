    ...
    if ($token_details['status'] == 'success') {
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($question_data = mysqli_fetch_assoc($result)) {
                $student_group_id = $question_data['student_group_id'];
                ...
                $test_array[$question_id] = array(
                    ...
                    "student_group" => !empty($student_group_id) ? $student_groups[$student_group_id] : '',
        ...
        $query_test_result = "SELECT DISTINCT(test_id) FROM test_result WHERE student_id = '$student_id' ORDER BY id DESC";
