...
if (count($error_msgs) == 0) {
    ...
    if ($token_details['status'] == 'success') {
        ...
        $query_total_tests = "SELECT id FROM self_assessor_test_config WHERE status = 1 AND created_by = '$student_id' $where_condition";
        ...
        $query_topic = "SELECT * FROM topic";
        $result_topic = mysqli_query($db, $query_topic);
        $topic = array();
        if (mysqli_num_rows($result_topic) > 0) {
            while ($topic_data = mysqli_fetch_assoc($result_topic)) {
                $topic_id = $topic_data['id'];
                $topic[$topic_id] = $topic_data['name'];
            }
        }

        $query = "SELECT * FROM self_assessor_test_config WHERE status = 1 AND created_by = '$student_id' $where_condition_for_final";
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($question_data = mysqli_fetch_assoc($result)) {
                ...
                $test_obj = array(
                    ...
                    "topic" => $ques_topic_id && isset($topic[$ques_topic_id]) ? $topic[$ques_topic_id] : '',
