    ...
    if ($token_details['status'] == 'success') {
        ...
        if (!empty($post_data->test_id)) {
            ...
            if (
                ...
                $query = "SELECT * FROM test_config WHERE name = '$test_name' AND status = 1 AND id != $test_id";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    $query = "UPDATE test_config SET name = '$test_name', category_id = '$category_id', 
                                instruction_id = '$instruction_id', duration = '$duration', difficulty_level = '$diffifulty_level', 
                                total_question = '$total_questions' WHERE id = '$test_id'";
                    $result = mysqli_query($db, $query);
