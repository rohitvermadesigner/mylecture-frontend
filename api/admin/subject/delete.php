            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    echo json_encode(array(
                        "message" => "$total_questions questions are associated with $subject_name so you can not delete $subject_name",
                    ));
                    return false;
                } else {
                    $current_date = date('Y-m-d H:i:s');
                    $query = "UPDATE subject SET status = '0', updated_at = '$current_date', deleted_at = '$current_date', deleted_by = '$admin_id' WHERE id = $subject_id";
                    $result = mysqli_query($db, $query);
                ...
                array_push($error_msgs, "Subject id is not valid");
            }
        } else {
            array_push($error_msgs, "Subject Id should not be empty");
        }
    } else {
