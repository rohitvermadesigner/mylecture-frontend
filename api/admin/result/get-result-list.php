...
if (count($error_msgs) == 0) {
    ...
    if ($token_details['status'] == 'success') {
        ...
        if (isset($_GET['test_id']) && !empty($_GET['test_id'])) {
            ...
            $query_student = "SELECT * FROM student_info WHERE status = 1";
            $result_student = mysqli_query($db, $query_student);
            $students = array();
            if (mysqli_num_rows($result_student) > 0) {
                while ($student_data = mysqli_fetch_assoc($result_student)) {
                    $student_id = $student_data['id'];
                    $students[$student_id] = $student_data['name'];
                }
            }
            ...
            $query = "SELECT * FROM $test_result_table WHERE test_id = '$test_id' ORDER BY created_at DESC";
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($result_data = mysqli_fetch_assoc($result)) {
                    ...
                    $test_obj = array(
                        ...
                        "student" => isset($student_id) ? $students[$student_id] : '',
