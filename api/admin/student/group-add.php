...
if (count($error_msgs) == 0) {
    ...
    if ($token_details['status'] == 'success') {
        ...
        if (!empty($post_data->name)) {
            ...
            $query = "SELECT * FROM student_group WHERE name = '$group_name' AND status = 1";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                $query = "INSERT INTO student_group (name, description, created_by, created_at) VALUES ('$group_name', '$description', '$admin_id', '$current_date')";
                $result = mysqli_query($db, $query);
            ...
            array_push($error_msgs, "group name should not be empty.");
        }
    } else {
